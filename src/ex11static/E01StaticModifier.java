...
class MyStatic {
	...
	/*
	인스턴스형 멤버메소드] 인스턴스형 메소드에서는 모든 멤버를 사용할수 있다.
		인스턴스형, 정적변수 둘다 접근가능함.
	 */
	void instanceMethod() {
	...
	정적 멤버메소드] 정적메소드 안에서는 인스턴스 멤버를 사용할수 없다.
		정적 멤버만 접근 가능하다.
		(※main메소드에서는 static으로 선언된 메소드만 호출할수 있었음을 기억하자)
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		방법2 : 일반적인 멤버변수처럼 참조변수를 통해 접근하는것도 가능하다.
			단, 이경우는 정적변수를 일반적인 멤버변수로 착각할수 있으므로 JAVA에서는 권장하지 않는다.
		 */
		...
		myStatic.staticMethod(); //권장사항이 아니므로 클래스명을 통해 접근하는것이 좋음.

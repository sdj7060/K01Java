...
public class QuArrayList {
	...
	public static void main(String[] args) {
		...
		for(Student st : list) {
			...
			//검색결과 있을때…검색된 데이터 삭제
			/*
			st.getName()를 이용해서 이름을 가져온다.
			 */
			if (name1.equals(st.getName())) {
		...
		if (index==-1) {
			...
			System.err.println("객체 삭제후 정보출력");
			/*
			컬렉션에 저장된 객체를 인덱스를 통해 삭제하면
			삭제된 객체 자체가 반환하게 되므로 멤버메소드를
			바로 호출할 수 있다. 
			 */
		}
		list.remove(index).show();
		//4.전체정보 출력
		System.out.println("검색후 전체 정보 출력");
		for(Student st : list) {

...
class Car {
	...
	Human owner;/*
		소유주 : Human클래스를 기반으로 하는 멤버변수로
			초기화를 위해 이름, 나이, 에너지 세가지의 값이 필요함.
			이와같이 클래스를 구성할때 또 다른 객체를 멤버변수로 선언할수 있다.
	*/
	...
	/*
	객체의 초기화를 담당하는 메소드로 아래는 항상 같은 값으로만 초기화된다는 한계를 가지고있다.
	 */
	void initialize() {
		carModel = "람보르기니";
		carYear = 2017;
	...
	void initialize(String model, int year, String name, int age, int energy) {
		...
		carYear = year;
		/*
		또 다른 객체가 멤버변수로 정의되었으므로, 객체를 생성한 후 각각의 멤버변수를 초기화해야한다.
		 */
		owner = new Human();
		owner.name = name;
		owner.age = age;
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		/*
			...
			초기화되지 않은 상태에서 출력을 시도하여 런타임에러가 발생한다.

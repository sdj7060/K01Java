...
-자동으로 호출되며, 개발자가 임의로 호출할수 없다.
-반환값이 없다. 즉 특정 연산후 값을 반환할수 없다.
	따라서 void를 쓰지 않는다.
-그외는 메소드가 가진 모든 특징을 가지고있다. (매개변수전달, 오버로딩 등)
...
용도1 : 동일 클래스 내에서 다른 생성자를 호출할때 사용한다.
	...
	사용법 : 
		this(인자1, 인자2,.....);
용도2 : 생성자내에서 멤버변수와 매개변수의 이름이 동일할때 구분하기 위해 사용된다.
	this.변수 => 멤버변수
	변수 => 생성자의 매개변수
...
public class PersonConstructor {
	...
	public PersonConstructor(String name) {
		...
		this.name = name;//좌측항은 멤버변수, 우측항은 매개변수
		age = 1;
		addr = "출처불명";
	...
	public PersonConstructor(String name, int age) {
		/*
		 ...
		 단, 생성자내에서만 사용가능하고, 일반 멤버메소드에서는 사용이 불가능하다.
		 */
		this(name, age, "미상");
